...
// "Added Photon networking for multiplayer movement and visibility" || version 1.0.2

using System.Collections;
using System.Collections.Generic;
using UnityEngine;
using UnityEngine.UI;
using Photon.Pun; // Added for Photon
...
public class FirstPersonController : MonoBehaviourPun // Changed to MonoBehaviourPun
{
    ...
    void Start()
    {
        // Disable camera and UI for non-local players
        ...
        {
            crosshairObject.enabled = photonView.IsMine;
        }
        if (sprintBarCG != null)
        {
            sprintBarCG.gameObject.SetActive(photonView.IsMine);
        }
        ...
        if (photonView.IsMine && crosshair)
        {
        ...
        #region Sprint Bar
        if (photonView.IsMine && useSprintBar)
        {
        ...
        Debug.Log("Player ViewID: " + photonView.ViewID + ", IsMine: " + photonView.IsMine);
    }
    ...
    void FixedUpdate()
    {
        if (!photonView.IsMine) return; // Only local player moves Rigidbody
    ...
    {
        if (!photonView.IsMine) return; // Only local player checks ground
    ...
    private void Crouch()
    {
        ...
        // Sync crouch state to all clients
        photonView.RPC("SyncCrouch", RpcTarget.Others, isCrouched);
    }

    [PunRPC]
    private void SyncCrouch(bool crouched)
    {
        isCrouched = crouched;
        transform.localScale = crouched
            ? new Vector3(originalScale.x, crouchHeight, originalScale.z)
            : new Vector3(originalScale.x, originalScale.y, originalScale.z);
    }

    private void HeadBob()
    {
        if (!photonView.IsMine) return; // Only local player applies head bob
        if (isWalking)
        {
            if (isSprinting)
            {
...
{
    // [Rest of the Custom Editor code remains unchanged]
    // Note: Ensure the editor script is in an Editor folder to avoid compilation issues
}
#endif
